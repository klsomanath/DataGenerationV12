function [TFD,t,f] = FTCWD(x,t,N,g,h,sigma,trace,sizeImg)
...
%	N     : number of frequency bins (defaultj : length(X)).
...
% if (xcol==1), tfr=real(P1); end
if (nargout==0)
%     tfrqview(tfr,x,t,'tfrcw',g,h,sigma);
elseif (nargout==3)
      f=(0.5*(0:N-1)/N)';
end
img = normalize(tfr,'range',[0 255]);
TFD = uint8(imresize(img,[sizeImg sizeImg]));